class RebalancingSearchTree {
  ...
  toString() {
    ...
    for (var rowIndex = 1; rowIndex < rows.length; rowIndex++) {
      ...
      for (var childIndex = 0; childIndex < rows[rowIndex].length; childIndex++) {
        ...
        if (childNode.value < parentNode.value) {
          ...
          str += (spacesForRow(rowIndex, rows.length) + childNode.value + spacesForRow(rowIndex, rows.length));
          parentIndex++;
        }
...
var spacesForRow = function(index, total) {
  return nSpaces(Math.pow(2, 2 + total - index) - 3);
};
